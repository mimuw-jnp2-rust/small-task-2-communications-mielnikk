...
impl Client {
        ...
        match self.connections.get_mut(addr) {
            ...
            None | Some(Closed) => {
                ...
                match res {
                    Err(e) => Err(e),
                    Ok(_) => {
                        self.connections.insert(addr.to_string(), Open(s));
                        Ok(())
                    }
                }
            }
    ...
    fn send(&mut self, addr: &str, msg: Message) -> CommsResult<Response> {
        match self.connections.get_mut(addr) {
            ...
            Some(Closed) => Err(ConnectionClosed(addr.to_string())),
            Some(Open(server)) => {
...
impl Server {
    ...
    fn receive(&mut self, msg: Message) -> CommsResult<Response> {
        ...
        let client = self.connected_client.borrow();

        match msg.msg_type {
            Handshake => match client {
                None => {
